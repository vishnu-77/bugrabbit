...
- **−** CI's label step reads the review's markdown table by **column position** (rubric order:
  `# | severity | category | location | scenario | fix | verdict`), not a structured field — it can
  miss a finding whose table row is formatted unexpectedly. It fails soft (no labels applied, never
  fails the job) rather than fabricating a wrong label, but it's a heuristic, not a guarantee.

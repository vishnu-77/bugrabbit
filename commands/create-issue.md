...
1. Parse a `<title>` (required) and optional `--body`, `--label`, `--autofix` (adds the `auto-fix`
   label so the cron poller and `/autofix-issues` pick it up — GitHub only, Bitbucket issues have no
   label equivalent and `host.sh` ignores `--label` there with a note).

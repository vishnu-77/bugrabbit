...
- **STALE-DONE** — a row `DONE` but its issue is still open with no merged PR → flag for recheck. Use
  the row's `updated` timestamp to say *how* stale (e.g. "DONE 9 days ago, still open") rather than
  just flagging the mismatch.
- **STALE-PARTIAL** — a row `PARTIAL` whose *shipped slice* has no open or merged PR at all (i.e. the
  branch exists but nothing was ever proposed for it) → flag for recheck, same `updated`-based aging.
  An issue staying open with no PR yet for the *deferred remainder* is expected for `PARTIAL` and is
  not itself stale — only flag when the shipped slice itself looks abandoned.
